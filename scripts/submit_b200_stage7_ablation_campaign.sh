...
RESET="${WA_DILOCO_STAGE7_RESET:-1}"
SUBMIT_AGGREGATE="${WA_DILOCO_STAGE7_AGGREGATE:-1}"
...
if [[ ! -f configs/stage7/configs.txt ]]; then
  "${PYTHON_BIN}" scripts/generate_stage7_configs.py
fi

mapfile -t CONFIGS < configs/stage7/configs.txt
mkdir -p results/stage7

manifest="results/stage7/submitted-$(date -u +%Y%m%dT%H%M%S).tsv"
...
if [[ "${SUBMIT_AGGREGATE}" == "1" && -n "${previous_job}" ]]; then
  aggregate_job_id="$(sbatch --parsable --dependency="afterany:${previous_job}" slurm/stage7-ablation-aggregate.sbatch)"
  printf "%s\taggregate\t%s\t%s\n" "${aggregate_job_id}" "slurm/stage7-ablation-aggregate.sbatch" "results/stage7" | tee -a "${manifest}"
fi
